"""
Master Validation Runner v2
FIXES:
1. Fast vectorized Hamming distance
2. Proper controlled similarity for cache tests
"""
...
log_file = RESULTS_DIR / f"validation_run_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
print(f"="*60)
print(f"BINARY SEMANTIC CACHE - VALIDATION RUN v2 (FIXED)")
print(f"="*60)
...
# Precompute popcount lookup table for FAST Hamming distance
...
def hamming_distance_fast(query, codes):
    """FAST vectorized Hamming distance using lookup table."""
    xored = query ^ codes
    # View uint64 as uint8 bytes, lookup popcount, sum
    bytes_view = xored.view(np.uint8)
    return POPCOUNT_TABLE[bytes_view].reshape(codes.shape[0], -1).sum(axis=1)

def hamming_similarity_fast(query, codes, code_bits=256):
    """Normalized Hamming similarity (1 = identical)."""
    dist = hamming_distance_fast(query, codes)
    return 1.0 - (dist.astype(np.float32) / code_bits)
...
# Test 2: Lookup latency with FAST implementation
print("\n[2/3] Lookup Latency Test (FAST vectorized)")
n_entries = 100_000
n_queries = 100
...
# Warm up
_ = hamming_similarity_fast(queries[0:1], codes)

start = time.perf_counter()
for q in queries:
    _ = hamming_similarity_fast(q.reshape(1, -1), codes)
...
print(f"  Status: {'✓ PASS' if lookup_pass else '✗ FAIL'}")
...
s1_results = {
    ...
    "overall_pass": encode_pass and memory_pass and lookup_pass
}

with open(RESULTS_DIR / "s1_latency_results_v2.json", "w") as f:
    json.dump(s1_results, f, indent=2)

s1_pass = encode_pass and lookup_pass and memory_pass
print(f"\nSTAGE 1: {'✓ PASS' if s1_pass else '⚠ PARTIAL'}")
print()

# Stage 3: Minimal PoC with CORRECT similarity testing
print("="*60)
print("STAGE 3: Minimal PoC (FIXED)")
print("="*60)
...
class BinarySemanticCache:
    ...
    def lookup(self, embedding) -> Tuple[Optional[str], float]:
        ...
        similarities = hamming_similarity_fast(query_code, self._codes, self._code_bits)
...
# Run PoC tests with CONTROLLED similarity
print("\n[1/3] Test: Similar embedding (0.95 cosine) should hit cache")
...
# Create embedding with GUARANTEED 0.95 cosine similarity
emb2 = create_similar_embedding(emb1, target_cosine=0.95, seed=456)

# Verify actual cosine
actual_cosine = float(np.dot(emb1.flatten(), emb2.flatten()))
print(f"  Created embedding with cosine: {actual_cosine:.4f}")

# Different embedding (random)
np.random.seed(789)
...
print(f"  Cache hit: {response is not None}")
...
s3_results = {
    ...
    "timestamp": datetime.now().isoformat(),
    "tests": {
        "similar_hit": {"pass": test1_pass, "similarity": sim if test1_pass else None, "actual_cosine": actual_cosine},
...
with open(RESULTS_DIR / "s3_poc_results_v2.json", "w") as f:
...
print(f"  S1 Latency:     {'✓ PASS' if s1_pass else '⚠ PARTIAL'}")
...
all_pass = s1_pass and s3_pass
print(f"TECHNICAL VALIDATION: {'✓ PASS' if all_pass else '⚠ ISSUES FOUND'}")
print("="*60)
print(f"\nLog saved to: {log_file}")
print(f"S1 results: {RESULTS_DIR / 's1_latency_results_v2.json'}")
print(f"S3 results: {RESULTS_DIR / 's3_poc_results_v2.json'}")
